...
const createUser = catchAsync(async (req, res, next) => {
  ...
  await insertSql
    .query_data(req, next, nepar_user_post)
    .then(async (rows) => {
...
exports.loginUser = async (req, res, next) => {
  ...
  await db.query(
    `SELECT id,provider from nepar_users WHERE login_id='${login_id}'`,
    async function (err, result, fields) {
      ...
      if (result.length == 0) {
        ...
        if (
          ...
          return next(
            new appError(
              "please enter valid provider facebook,gmail or email",
              401
            )
          );
        ...
        if (db_provider != provider)
          return next(
            new appError("please specify the correct provider for the login_id")
          );
...
exports.uploadImage_profilePic = upload.uploadImage.array("images", 1);
...
exports.update_nepar_user_profilepic_db = catchAsync(async (req, res, next) => {
  upload.resizeImage(req, next, 500, 500, 100, `public/images/profile_pics`).then(async (data) => {
    req.query = `UPDATE nepar_users SET ? WHERE id=${req.params.nepar_user_id} `;
    await Promise.all(
      req.body.images.map(async (filename, i) => {
        console.log(filename)
        await query.query_data(req, next, {
          profile_pic: `${req.protocol}://${req.get("host")}/images/profile_pics/${filename}`
        });
      })
    );
    res.status(200).json({
